...
and death~\citep{Kohn1999,James2013}.
%
...
death associated with opioids in hospitalized patients
\citep{Belknap2008}.

To adapt software engineering techniques to implement and maintain
highly reliable \emph{electronic} prescriptions, the PIs have been
designing and building the Patient-Oriented Prescription Programming
Language (\poppl{}).
%
In this \poppl{} view, clinicians who write prescriptions face the same
spectrum of problems as professional programmers who write software,
and yet clinicians must be able to write prescriptions without
extensive formal training in programming.
%
\emph{Domain-specific languages} (DSLs), which tailor a programming task to
the structure and vocabulary of a domain expert, offer a tractable
resolution to this mismatch.
%
A DSL for prescriptions should build on existing concepts, such as
order sets, but give clinicians the expressive power of abstractions
over those concepts---and it should provide a smooth path for
clinicians to take advantage of more abstraction as they become more
comfortable with writing programs.
